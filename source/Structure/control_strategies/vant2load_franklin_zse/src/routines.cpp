...
Eigen::MatrixXd routines::generate_AerodynDisturbances(double xdot, double ydot, double zdot, double phi, double theta,
                                                       double psi, Eigen::MatrixXd wind)
{
  // Air density
...
Eigen::MatrixXd routines::generate_Wind(int k, double Ts, int which_wind)
{
  // k = Time instant k
  double t = k * Ts;
                                    ...
                                    simulator_msgs::Sensor rody, std::vector<double>& y, std::vector<int>& I)
{
  ...
  static std::default_random_engine gen15(rand());  // One generator for each sensor, each one with a different seed
                                                    // generated by rand(). Trying to ensure independency
...
void routines::generate_Measurement_truncated(int k, double Ts, simulator_msgs::Sensor mainbody,
                                              simulator_msgs::Sensor rightpropeller,
                                              ...
                                              std::vector<double>& y, std::vector<int>& I)
{
  ...
  if (Barometer_noise < -3 * 0.17)
  {
    Barometer_noise = -3 * 0.17;
  }
  if (Barometer_noise > 3 * 0.17)
  {
    Barometer_noise = 3 * 0.17;
  }
  if (IMU_phi_noise < -3 * (0.05 * pi / 180))
  {
    IMU_phi_noise = -3 * (0.05 * pi / 180);
  }
  if (IMU_phi_noise > 3 * (0.05 * pi / 180))
  {
    IMU_phi_noise = 3 * (0.05 * pi / 180);
  }
  if (IMU_theta_noise < -3 * (0.05 * pi / 180))
  {
    IMU_theta_noise = -3 * (0.05 * pi / 180);
  }
  if (IMU_theta_noise > 3 * (0.05 * pi / 180))
  {
    IMU_theta_noise = 3 * (0.05 * pi / 180);
  }
  if (IMU_psii_noise < -3 * (0.05 * pi / 180))
  {
    IMU_psii_noise = -3 * (0.05 * pi / 180);
  }
  if (IMU_psii_noise > 3 * (0.05 * pi / 180))
  {
    IMU_psii_noise = 3 * (0.05 * pi / 180);
  }
  if (IMU_p_noise < -3 * 0.005519215703220)
  {
    IMU_p_noise = -3 * 0.005519215703220;
  }
  if (IMU_p_noise > 3 * 0.005519215703220)
  {
    IMU_p_noise = 3 * 0.005519215703220;
  }
  if (IMU_q_noise < -3 * 0.005519215703220)
  {
    IMU_q_noise = -3 * 0.005519215703220;
  }
  if (IMU_q_noise > 3 * 0.005519215703220)
  {
    IMU_q_noise = 3 * 0.005519215703220;
  }
  if (IMU_r_noise < -3 * 0.005519215703220)
  {
    IMU_r_noise = -3 * 0.005519215703220;
  }
  if (IMU_r_noise > 3 * 0.005519215703220)
  {
    IMU_r_noise = 3 * 0.005519215703220;
